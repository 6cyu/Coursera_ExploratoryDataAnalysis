...
# subsetting the NEI data for Baltimore city (fips=24510) and LA (fips=06037)
NEI_Balt_LA_Veh <- subset(NEI,(SCC %in% SCC_veh$SCC & fips=="24510")|
                                  (SCC %in% SCC_veh$SCC & fips=="06037"))

# converting the country's code
NEI_Balt_LA_Veh2 <- NEI_Balt_LA_Veh
NEI_Balt_LA_Veh2$fips[NEI_Balt_LA_Veh2$fips=="06037"] <- "Los_Angeles"
NEI_Balt_LA_Veh2$fips[NEI_Balt_LA_Veh2$fips=="24510"] <- "Baltimore"

## plotting ggplot2
library(ggplot2)
g <- ggplot(NEI_Balt_LA_Veh2, aes(x = year, y = Emissions))
p <- g + geom_point(aes(color=type), size = 3)+
        ...
        theme(text = element_text(size = 20),
              axis.text.x = element_text(colour="gray20", angle = 90),
              axis.text.y = element_text(colour="gray20"))
print(p)

## creating plot5.png
dev.copy(png, file="plot6.png", width = 800)
dev.off()